...
| A.8.5 | Secure authentication | Additional | ✅ | MFA is mandatory for access to systems processing health-related personal data; where native MFA is unsupported (CMS), static IP whitelisting applies as a documented, risk-accepted compensating control (Risk #28). | Team Lead Developer |
| A.8.13 | Information backup | Additional | ✅ | Health data backups encrypted, retained per legal requirements, recoverable within 4-hour RTO; monthly restore tests (pass/fail). | Team Lead Developer |
...
| A.5.39 | HLT — Unique identification of care recipients | HLT | ⚠️ Partial (by design) | Deliberate scope boundary, not an implementation gap. Rate is a feedback platform: it ensures feedback is reliably attributed to the submitting respondent (data integrity), but unique clinical patient identification is the healthcare customer's responsibility as data controller. This split is formalized in the BOZ addendum and General Terms & Conditions (IMP-2026-NEN-01, closed). No further action required by Rate. | Healthcare Customer / Team Lead Developer |

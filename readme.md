...
| **6.3 Internet Outage at CEO's Home (Network / VPN Unavailable)** | When the home network is down, VPN access is unavailable. Access the NAS and development server physically on-site. Restore production access by whitelisting the active connection IP on the Hetzner firewall dashboard. See Alternative Access Method Test Log (Logs §12). | Managing Director / Team Lead Developer | Immediate | None |
